...
function compareWords() {
    ...
    //if (greenIndices.length === 4 || wordCount === 6) {
	if (greenIndices.length === 4) {
        win = true;
        myConfetti({
            particleCount: 100,
            startVelocity: 30,
        ...
        openNotificationLong("כל הכבוד הצלחת ! שמו בישראל: עמית", true);
        //openNotificationLong(winMessage, true);

        openShareNotificationLong("שתף תוצאה", true);


    }
    //if ended and lost:
    if (wordCount === 6 && greenIndices.length != 4) {
        endOfGameToday = true;
        let message = `לא נורא, עשו ריפרש ונסו שוב`;
        openNotificationLong(message, false);
    }
	console.log(greenIndices.length);
	console.log(yellowIndices.length);
}
function pickMessage() {
    ...
    if (wordCount === 2) {
        messageArray = ['נסכם את ההצלחה בשתי מילים: יופי טופי','טובים השניים מן האחד (ניחושים כאילו)', 'גאוני', 'אמאל׳ה הצלחת מהר', 'אחלה תוצאה שבעולם', 'נראה שהלך מדהים', '!!!טירוף', 'שני ניחושים? קטונתי', 'יש לך מוח ענק', 'תוצאה מפחידה', 'וואו פשוט וואו', 'בגלגול הקודם היית מילונאי', "שני ניחושים יצאו לדרך בים בם בום", 'אין לנו מה לומר, הצלחת', 'מזל של מתחילים', 'הלכת אול אין וזה השתלם אחושקשוקי', 'לא רואים אבל אני משתחווה','מוצלח, מאוד מאוד מוצלח']
    }
    if (wordCount === 3) {
